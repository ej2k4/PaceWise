...
def read_aloud(text: str):
    try:
        ...
        # Try to find a more "chill" voice (e.g., Zira)
        voices = speaker.GetVoices()
        for i in range(voices.Count):
            if "Zira" in voices.Item(i).GetDescription() or "Female" in voices.Item(i).GetDescription():
                speaker.Voice = voices.Item(i)
                break
        
        # Slow down slightly for a more relaxed, gentle tone
        speaker.Rate = -1 
        
        print("\n[Reading story aloud...]")
        speaker.Speak(text)
    except ImportError:
        print("\n[Read Aloud] pywin32 not installed. Skipping voice output.")
