...
def register_student(name: str, password: str, age: int = 5) -> dict:
    student_id = str(uuid.uuid4())
    with get_cursor() as cur:
        # Check if name already exists (simple auth)
        cur.execute("SELECT * FROM students WHERE name = %s", (name,))
        if cur.fetchone():
            raise ValueError("Student name already exists")
            
        cur.execute("""
            INSERT INTO students (student_id, name, password, age)
            VALUES (%s, %s, %s, %s)
            RETURNING *
        """, (student_id, name, password, age))
        return dict(cur.fetchone())

def login_student(name: str, password: str) -> Optional[dict]:
    with get_cursor() as cur:
        cur.execute("SELECT * FROM students WHERE name = %s AND password = %s", (name, password))
        row = cur.fetchone()
        return dict(row) if row else None
