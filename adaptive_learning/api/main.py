...
def _fmt(row: dict) -> QuestionOut:
    ...
    if opts:
        ...
        opts = {k: v for k, v in zip(keys, values)}
...
def auth_register(req: AuthRequest):
    try:
        student = register_student(req.name, req.password, req.age)
        return {"message": "Success", "student": {"student_id": student["student_id"], "name": student["name"], "age": student["age"]}}
...
def auth_login(req: AuthRequest):
    student = login_student(req.name, req.password)
    if not student:
...
def submit_answer(req: AnswerRequest):
    ...
    stats = get_student_stats(req.student_id, req.module, req.topic) or {}

    stats.setdefault("past_accuracy_topic", 0.5)
    stats.setdefault("past_accuracy_module", 0.5)
    stats.setdefault("attempts_on_topic", 0)
    stats.setdefault("session_number", 1)
    stats.setdefault("time_since_last_sec", 0)
